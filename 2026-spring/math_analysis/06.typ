...
$
よって
$limsup_(m -> infinity) a_m / m <= a_n / n$ (左は上極限)

これは任意の$n$について成り立つので
$
  limsup_(m -> infinity) a_m / m <= inf_(n >= 1) a_n / n
$
である.
一方,一般に
$
  inf_(n >= 1) a_n / n <= liminf_(n -> infinity) a_n / n <= limsup_(n -> infinity) a_n / n
$
である.
したがって
$
  limsup_(n -> infinity) a_n / n <= liminf_(n -> infinity) a_n / n
$
となる.
常に$liminf <= limsup$なので両者は一致し,$a_n/n$は収束する.

しかも$a_n >= 0$より極限は$0$以上であり,$a_1/n$などで上から抑えられるので有限である.
...
$
  lim_(n -> infinity) chi_n(G) / n
$
が存在する.
この極限は分数彩色数と呼ばれる量につながる.
...
$
  Y_N = 1/N sum_(i=1)^N f(X_i)
$
とおく.
$mu = E[f(X)]$, $sigma^2 = V[f(X)]$とすると,
$
  E[Y_N] = mu
$
であり,
$
  V[Y_N]
  = V(1/N sum_(i=1)^N f(X_i))
  = 1/N^2 sum_(i=1)^N V[f(X_i)]
  = sigma^2 / N
$
となる.
したがって標準偏差は
...
$
である.
これがモンテカルロ法の誤差が$sqrt(N)$のオーダーでしか小さくならない理由である.
...
モンテカルロ法の標準偏差は$O(1 / sqrt(N))$の速さで小さくなる.
次元に直接依存しないので,高次元ではリーマン和より有利になることが多い.
...
$dif x /dif t = r (1 - x / K) x$

$x$が小さいときは$dif x/dif t approx r x$で指数的に増えるが,
$x$が$K$に近づくと増加率が$0$に近づく.
これをロジスティック方程式という.
